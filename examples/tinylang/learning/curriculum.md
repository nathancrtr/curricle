...
- **Read:** [Crafting Interpreters](res:crafting), the "Scanning" chapter, all of it — it is the closest thing to a canonical treatment. Then look at [Python's `tokenize` module](res:tokenize) output for a small Python file (`python -m tokenize somefile.py`) to see what a production lexer emits and how much more of it there is than you expected.
